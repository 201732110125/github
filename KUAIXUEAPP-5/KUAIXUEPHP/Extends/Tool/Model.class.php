...
//实现  M('users')->add();自动添加数据（根据表单提交过来的数据）
public function add($data=null){
  if(is_null($data))  $data=$_POST;//取出$_POST内的数据
  $fields='';
  $values='';
  foreach($data as $f =>$v){//组合成insert语句中的条件语句
  ...
  $sql="INSERT INTO ".$this->table.' ( '.$fields.' ) VALUES ('.$values.')';//组装
  return $this->exe($sql);//发送给exe()方法

}
//对数据的更新
public function update($data=null){
  if(empty($this->opt['where'])){
    halt("更行语句必须要有where条件");
  }
  if(is_null($data)){
    $data=$_POST;
  }
  //update table_name set colunms_1=value_1,colunms_2=value_2;
  $values='';
  foreach($data as $f=>$v){
